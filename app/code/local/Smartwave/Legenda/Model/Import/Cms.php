...
class Smartwave_Legenda_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('legenda')->__("Can't get the data file for import cms blocks/pages: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('legenda')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('legenda')->__('No items were imported')
				);
			}
