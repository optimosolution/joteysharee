	...
	*/
	...
	public function getPriceDisplayType(){
		...
		if($this->isTextBoxEnabled()){
			$html = '
				<div class="text-box"><input type="text" name="min" id="minPrice" class="input-text priceTextBox minPrice f-left" value="'.$this->getCurrMinPrice().'" style="'.$textBoxStyle.'" /> 
					<span class="seperate-dash f-left">&ndash;</span><input type="text" name="max" id="maxPrice" class="input-text priceTextBox maxPrice f-left" value="'.$this->getCurrMaxPrice().'" style="'.$textBoxStyle.'" />
            ...
            $html = '<div class="price_slider_label"> Price: ';
	...
	*/
	...
	public function prepareCustomStyles(){
		...
		$html = '<style type="text/css">';	
			...
			$html .= 'width:'.$handleWidth.'px; height:'.$handleHeight.'px; margin-top: -.1em; cursor: pointer; }';
			
			$html .= '.ui-slider{';
			if($bgSlider){$html .= 'background:'.$bgSlider.';';}
            if (is_integer($sliderWidth)) $sliderWidth .= 'px';
			$html .= ' width:'.$sliderWidth.'; height:'.$sliderHeight.'px; border:none; margin-bottom: 15px; }';
